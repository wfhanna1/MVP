...
export class AddPlayer extends Component {
  ...
  onValidate() {
    ...
    if (!ValidationService.nonEmptyString(this.state.fullName))
      messages.push("A name is required.");
    if (this.state.fullName.length > 255)
      messages.push("Full name must be < 255 characters");
    if (!ValidationService.email(this.state.emailAddress))
      messages.push("Enter a valid email address");
    if (
      ValidationService.nonEmptyString(this.state.profilePhoto) &&
      !ValidationService.url(this.state.profilePhoto)
    ) {
  ...
  async onSubmit() {
    ...
    const player = new Player({
      ...
      profilePhoto:
        this.state.profilePhoto.length > 0 ? this.state.profilePhoto : null
    });
